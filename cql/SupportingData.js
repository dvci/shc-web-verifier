...
const validator = (xpath, currentValue, newValue) => {
  ...
  if (
    [
      ...
      '/antigenSupportingData/series/seriesDose/inadvertentVaccine'
    ].includes(xpath)
  ) {
  ...
  if (
    [
      '/antigenSupportingData/series/selectSeries/minAgeToStart',
      ...
      '/antigenSupportingData/series/seriesDose/allowableVaccine/endAge'
    ].includes(xpath)
  ) {
  ...
  if (
    [
      '/antigenSupportingData/series/seriesDose/interval/effectiveDate',
      ...
      '/antigenSupportingData/series/seriesDose/allowableInterval/cessationDate'
    ].includes(xpath)
  ) {
...
xml2js.parseString(xml, { explicitArray: false, validator }, (err, result) => {
  if (err) {
    throw err;
  }
