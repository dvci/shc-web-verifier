  ...
  function T(a, b) { const c = []; let d = ''; b = a.readBits([8, 16, 16][b]); for (let d = 0; d < b; d++) { const b = a.readBits(8); c.push(b) } try { d += decodeURIComponent(c.map((a) => `%${(`0${a.toString(16)}`).substr(-2)}`).join('')) } catch (e) {} return { bytes: c, text: d } } function U(a, b) {
    a = new V(a); b = b <= 9 ? 0 : b <= 26 ? 1 : 2; const c = { text: '', bytes: [], chunks: [] }; for (;a.available() >= 4;) {
      let d = a.readBits(4); if (d === y.Terminator) return c; if (d === y.ECI) {
        a.readBits(1) === 0 ? c.chunks.push({ type: t.ECI, assignmentNumber: a.readBits(7) }) : a.readBits(1) === 0
          ? c.chunks.push({ type: t.ECI, assignmentNumber: a.readBits(14) }) : a.readBits(1) === 0 ? c.chunks.push({ type: t.ECI, assignmentNumber: a.readBits(21) }) : c.chunks.push({ type: t.ECI, assignmentNumber: -1 });
      } else if (d === y.Numeric) {
        var e = a; d = []; for (var f = '', g = e.readBits([10, 12, 14][b]); g >= 3;) { let h = e.readBits(10); if (h >= 1E3) throw Error('Invalid numeric value above 999'); var k = Math.floor(h / 100); var l = Math.floor(h / 10) % 10; h %= 10; d.push(48 + k, 48 + l, 48 + h); f += k.toString() + l.toString() + h.toString(); g -= 3 } if (g === 2) {
          g = e.readBits(7); if (g
>= 100) throw Error('Invalid numeric value above 99'); e = Math.floor(g / 10); g %= 10; d.push(48 + e, 48 + g); f += e.toString() + g.toString()
        } else if (g === 1) { e = e.readBits(4); if (e >= 10) throw Error('Invalid numeric value above 9'); d.push(48 + e); f += e.toString() }d = { bytes: d, text: f }; c.text += d.text; c.bytes.push(...d.bytes); c.chunks.push({ type: t.Numeric, text: d.text })
      } else if (d === y.Alphanumeric) {
        e = a; d = []; f = ''; for (g = e.readBits([9, 11, 13][b]); g >= 2;) {
          l = e.readBits(11), k = Math.floor(l / 45), l %= 45, d.push(B[k].charCodeAt(0), B[l].charCodeAt(0)),
          f += B[k] + B[l], g -= 2;
        }g === 1 && (e = e.readBits(6), d.push(B[e].charCodeAt(0)), f += B[e]); d = { bytes: d, text: f }; c.text += d.text; c.bytes.push(...d.bytes); c.chunks.push({ type: t.Alphanumeric, text: d.text })
      } else if (d === y.Byte)d = T(a, b), c.text += d.text, c.bytes.push(...d.bytes), c.chunks.push({ type: t.Byte, bytes: d.bytes, text: d.text }); else if (d === y.Kanji) {
        ...
        d = { bytes: d, text: f }; c.text += d.text; c.bytes.push(...d.bytes); c.chunks.push({ type: t.Kanji, bytes: d.bytes, text: d.text })
      }
    ...
    c = aa[c.dataMask]; const d = a.height; let e = 17 + 4 * b.versionNumber; let f = A.createEmpty(e, e); f.setRegion(0, 0, 9, 9, !0); f.setRegion(e - 8, 0, 8, 9, !0); f.setRegion(0, e - 8, 9, 8, !0); for (var g of b.alignmentPatternCenters) for (var h of b.alignmentPatternCenters)g === 6 && h === 6 || g === 6 && h === e - 7 || g === e - 7 && h === 6 || f.setRegion(g - 2, h - 2, 5, 5, !0); f.setRegion(6, 9, 1, e - 17, !0); f.setRegion(9,
      6, e - 17, 1, !0); b.versionNumber > 6 && (f.setRegion(e - 11, 0, 3, 6, !0), f.setRegion(0, e - 11, 6, 3, !0)); b = f; g = []; e = h = 0; f = !0; for (let k = d - 1; k > 0; k -= 2) { k === 6 && k--; for (let l = 0; l < d; l++) { const q = f ? d - 1 - l : l; for (let d = 0; d < 2; d++) { const f = k - d; if (!b.get(f, q)) { e++; let b = a.get(f, q); c({ y: q, x: f }) && (b = !b); h = h << 1 | b; e === 8 && (g.push(h), h = e = 0) } } }f = !f } return g
  } function ba(a) {
...
< b; c++)e[g + c].codewords.push(a.shift());
  ...
  function M(a, b, c, d) {
    ...
    } { const h = b.x - c.x; let g = d.x - c.x; const k = b.y - c.y; const l = d.y - c.y; c = h * l - g * k; g = (e * l - g * f) / c; e = (h * f - e * k) / c; return {
...
* c.a23; const k = d.a12 * c.a21 + d.a22 * c.a22 + d.a32 * c.a23; const l = d.a13 * c.a21 + d.a23 * c.a22 + d.a33 * c.a23; const q = d.a11 * c.a31 + d.a21 * c.a32 + d.a31 * c.a33; const p = d.a12 * c.a31 + d.a22 * c.a32 + d.a32 * c.a33; const m = d.a13 * c.a31 + d.a23 * c.a32 + d.a33 * c.a33; c = A.createEmpty(b.dimension, b.dimension); d = (a, b) => { const c = g * a + l * b + m; return { x: (e * a + h * b + q) / c, y: (f * a + k * b + p) / c } }; for (let e = 0; e < b.dimension; e++) for (let f = 0; f < b.dimension; f++) { const b = d(f + 0.5, e + 0.5); c.set(f, e, a.get(Math.floor(b.x), Math.floor(b.y))) } return { matrix: c, mappingFunction: d }
  } function u(a) {
    return a.reduce((a,
      c) => a + c)
  } function ia(a, b, c) { const d = x(a, b); const e = x(b, c); const f = x(a, c); let g; let h; let k; e >= d && e >= f ? [g, h, k] = [b, a, c] : f >= e && f >= d ? [g, h, k] = [a, b, c] : [g, h, k] = [a, c, b]; (k.x - h.x) * (g.y - h.y) - (k.y - h.y) * (g.x - h.x) < 0 && ([g, k] = [k, g]); return { bottomLeft: g, topLeft: h, topRight: k } } function ja(a, b, c, d) { d = (u(z(a, c, d, 5)) / 7 + u(z(a, b, d, 5)) / 7 + u(z(c, a, d, 5)) / 7 + u(z(b, a, d, 5)) / 7) / 4; if (d < 1) throw Error('Invalid module size'); b = Math.round(x(a, b) / d); a = Math.round(x(a, c) / d); a = Math.floor((b + a) / 2) + 7; switch (a % 4) { case 0: a++; break; case 2: a-- } return { dimension: a, moduleSize: d } }
  function N(a, b, c, d) {
    const e = [{ x: Math.floor(a.x), y: Math.floor(a.y) }]; let f = Math.abs(b.y - a.y) > Math.abs(b.x - a.x); if (f) { var g = Math.floor(a.y); var h = Math.floor(a.x); a = Math.floor(b.y); b = Math.floor(b.x) } else g = Math.floor(a.x), h = Math.floor(a.y), a = Math.floor(b.x), b = Math.floor(b.y); const k = Math.abs(a - g); const l = Math.abs(b - h); let q = Math.floor(-k / 2); const p = g < a ? 1 : -1; const m = h < b ? 1 : -1; let r = !0; for (let n = g, v = h; n !== a + p; n += p) { g = f ? v : n; h = f ? n : v; if (c.get(g, h) !== r && (r = !r, e.push({ x: g, y: h }), e.length === d + 1)) break; q += l; if (q > 0) { if (v === b) break; v += m; q -= k } }c = []; for (f = 0; f < d; f++)e[f] && e[f + 1] ? c.push(x(e[f], e[f + 1])) : c.push(0); return c
  } function z(a, b, c, d) { const e = b.y - a.y; const f = b.x - a.x; b = N(a, b, c, Math.ceil(d / 2)); a = N(a, { x: a.x - f, y: a.y - e }, c, Math.ceil(d / 2)); c = b.shift() + a.shift() - 1; return a.concat(c).concat(...b) } function F(a, b) { const c = u(a) / u(b); let d = 0; b.forEach((b, f) => { d += Math.pow(a[f] - b * c, 2) }); return { averageSize: c, error: d } } function O(a, b, c) {
    try {
    ...
    let b = []; let c = []; let d = []; let e = []; for (let m = 0; m <= a.height; m++) {
      var f = 0; let l = !1; let n = [0, 0, 0, 0, 0]; for (let b = -1; b <= a.width; b++) {
        let g = a.get(b, m); if (g === l)f++; else {
          n = [n[1], n[2], n[3], n[4], f]; f = 1; l = g; let h = u(n) / 7; h = Math.abs(n[0] - h) < h && Math.abs(n[1] - h) < h && Math.abs(n[2] - 3 * h) < 3 * h && Math.abs(n[3] - h) < h && Math.abs(n[4] - h) < h && !g; let k = u(n.slice(-3)) / 3; g = Math.abs(n[2]
- k) < k && Math.abs(n[3] - k) < k && Math.abs(n[4] - k) < k && g; if (h) { const a = b - n[3] - n[4]; const d = a - n[2]; h = { startX: d, endX: a, y: m }; k = c.filter((b) => d >= b.bottom.startX && d <= b.bottom.endX || a >= b.bottom.startX && d <= b.bottom.endX || d <= b.bottom.startX && a >= b.bottom.endX && n[2] / (b.bottom.endX - b.bottom.startX) < 1.5 && n[2] / (b.bottom.endX - b.bottom.startX) > 0.5); k.length > 0 ? k[0].bottom = h : c.push({ top: h, bottom: h }) } if (g) {
            const a = b - n[4]; const c = a - n[3]; g = { startX: c, y: m, endX: a }; h = e.filter((b) => c >= b.bottom.startX && c <= b.bottom.endX || a >= b.bottom.startX && c <= b.bottom.endX
|| c <= b.bottom.startX && a >= b.bottom.endX && n[2] / (b.bottom.endX - b.bottom.startX) < 1.5 && n[2] / (b.bottom.endX - b.bottom.startX) > 0.5); h.length > 0 ? h[0].bottom = g : e.push({ top: g, bottom: g })
          }
        }
      }b.push(...c.filter((a) => a.bottom.y !== m && a.bottom.y - a.top.y >= 2)); c = c.filter((a) => a.bottom.y === m); d.push(...e.filter((a) => a.bottom.y !== m)); e = e.filter((a) => a.bottom.y === m)
...
/ 4; const d = (b.top.y + b.bottom.y + 1) / 2; if (a.get(Math.round(c), Math.round(d))) {
        return b = [b.top.endX - b.top.startX, b.bottom.endX - b.bottom.startX, b.bottom.y - b.top.y + 1], b = u(b) / b.length, {
...
+ c[0].score + c[1].score; return { points: [a].concat(c.slice(0, 2)), score: d }
      ...
      .filter((a) => !!a)
      .sort((a, b) => a.score - b.score); if (b.length === 0) return null; const { topRight: l, topLeft: q, bottomLeft: p } = ia(b[0].points[0], b[0].points[1], b[0].points[2]); c = P(a, d, l, q, p); b = []; c && b.push({
      alignmentPattern: { x: c.alignmentPattern.x, y: c.alignmentPattern.y }, bottomLeft: { x: p.x, y: p.y }, dimension: c.dimension, topLeft: { x: q.x, y: q.y }, topRight: { x: l.x, y: l.y }
    }); c = H(a, l); e = H(a, q); f = H(a, p); (d = P(a, d, c, e, f)) && b.push({
      alignmentPattern: {
    ...
    let f; let g; try { ({ dimension: f, moduleSize: g } = ja(d, c, e, a)) } catch (q) { return null } const h = c.x - d.x + e.x; const k = c.y - d.y + e.y; c = (x(d, e) + x(d, c)) / 2 / g; e = 1 - 3 / c; const l = { x: d.x + e * (h - d.x), y: d.y + e * (k - d.y) }; b = b.map((b) => {
      const c = (b.top.startX + b.top.endX + b.bottom.startX + b.bottom.endX) / 4; const d = (b.top.y + b.bottom.y + 1) / 2; if (a.get(Math.floor(c), Math.floor(d))) {
        return u([b.top.endX
- b.top.startX, b.bottom.endX - b.bottom.startX, b.bottom.y - b.top.y + 1]), b = O({ x: Math.floor(c), y: Math.floor(d) }, [1, 1, 1], a) + x({ x: c, y: d }, l), { x: c, y: d, score: b }
      }
    }).filter((a) => !!a).sort((a, b) => a.score - b.score); return { alignmentPattern: c >= 15 && b.length ? b[0] : l, dimension: f }
  } function Q(a) {
        ...
        let d = L(c); if (d)c = d; else {
          for (d = 0; d < c.width; d++) {
            for (let a = d + 1; a < c.height; a++) {
              c.get(d, a) !== c.get(a, d) && (c.set(d, a, !c.get(d, a)), c.set(a, d, !c.get(a,
                d)));
            }
          }c = L(c)
        }
    ...
    const e = Object.create(null); R(e, la); R(e, d); d = e.inversionAttempts === 'onlyInvert' || e.inversionAttempts === 'invertFirst'; let f = e.inversionAttempts === 'attemptBoth' || d; let g = e.greyScaleWeights; let h = e.canOverwriteImage; let k = b * c; if (a.length !== 4 * k) throw Error('Malformed data passed to binarizer.'); let l = 0; if (h) { var q = new Uint8ClampedArray(a.buffer, l, k); l += k }q = new S(b, c, q); if (g.useIntegerApproximation) {
      for (var p = 0; p < c; p++) {
        for (var m = 0; m < b; m++) {
          var r = 4 * (p * b + m); q.set(m, p, g.red * a[r] + g.green
* a[r + 1] + g.blue * a[r + 2] + 128 >> 8)
        }
      }
    } else for (p = 0; p < c; p++) for (m = 0; m < b; m++)r = 4 * (p * b + m), q.set(m, p, g.red * a[r] + g.green * a[r + 1] + g.blue * a[r + 2]); g = Math.ceil(b / 8); p = Math.ceil(c / 8); m = g * p; if (h) { var n = new Uint8ClampedArray(a.buffer, l, m); l += m }n = new S(g, p, n); for (m = 0; m < p; m++) {
      for (r = 0; r < g; r++) {
        var v = Infinity; var t = 0; for (var u = 0; u < 8; u++) for (let a = 0; a < 8; a++) { const b = q.get(8 * r + a, 8 * m + u); v = Math.min(v, b); t = Math.max(t, b) }u = (v + t) / 2; u = Math.min(255, 1.1 * u); t - v <= 24 && (u = v / 2, m > 0 && r > 0 && (t = (n.get(r, m - 1) + 2 * n.get(r - 1, m) + n.get(r - 1, m - 1)) / 4, v < t && (u = t))); n.set(r, m, u)
      }
    }h ? (m = new Uint8ClampedArray(a.buffer, l, k), l += k, m = new A(m, b)) : m = A.createEmpty(b, c); r = null; f && (h ? (a = new Uint8ClampedArray(a.buffer, l, k), r = new A(a, b)) : r = A.createEmpty(b, c)); for (b = 0; b < p; b++) for (a = 0; a < g; a++) { c = g - 3; c = a < 2 ? 2 : a > c ? c : a; h = p - 3; h = b < 2 ? 2 : b > h ? h : b; k = 0; for (l = -2; l <= 2; l++) for (v = -2; v <= 2; v++)k += n.get(c + l, h + v); c = k / 25; for (h = 0; h < 8; h++) for (k = 0; k < 8; k++)l = 8 * a + h, v = 8 * b + k, t = q.get(l, v), m.set(l, v, t <= c), f && r.set(l, v, !(t <= c)) }f = f ? { binarized: m, inverted: r } : { binarized: m }; const { binarized: w, inverted: x } = f;
    (f = Q(d ? x : w)) || e.inversionAttempts !== 'attemptBoth' && e.inversionAttempts !== 'invertFirst' || (f = Q(d ? w : x)); return f
  } class A {
  ...
  } let t; (function (a) { a.Numeric = 'numeric'; a.Alphanumeric = 'alphanumeric'; a.Byte = 'byte'; a.Kanji = 'kanji'; a.ECI = 'eci' }(t || (t = {}))); let y; (function (a) {
  ...
  }(y || (y = {}))); let B = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ $%*+-./:'.split(''); class w {
...
- 1
    ...
    addOrSubtract(a) { if (this.isZero()) return a; if (a.isZero()) return this; let b = this.coefficients; a = a.coefficients; b.length > a.length && ([b, a] = [a, b]); const c = new Uint8ClampedArray(a.length); const d = a.length - b.length; for (var e = 0; e < d; e++)c[e] = a[e]; for (e = d; e < a.length; e++)c[e] = b[e - d] ^ a[e]; return new w(this.field, c) }

    multiply(a) {
      if (a === 0) return this.field.zero; if (a === 1) return this; const b = this.coefficients.length;
      const c = new Uint8ClampedArray(b); for (let d = 0; d < b; d++)c[d] = this.field.multiply(this.coefficients[d], a); return new w(this.field, c)
    }

    multiplyPoly(a) { if (this.isZero() || a.isZero()) return this.field.zero; const b = this.coefficients; const c = b.length; a = a.coefficients; const d = a.length; const e = new Uint8ClampedArray(c + d - 1); for (let h = 0; h < c; h++) { const c = b[h]; for (let b = 0; b < d; b++) { const f = h + b; const g = this.field.multiply(c, a[b]); e[f] = e[h + b] ^ g } } return new w(this.field, e) }

    multiplyByMonomial(a, b) {
      if (a < 0) throw Error('Invalid degree less than 0'); if (b
=== 0) return this.field.zero; const c = this.coefficients.length; a = new Uint8ClampedArray(c + a); for (let d = 0; d < c; d++)a[d] = this.field.multiply(this.coefficients[d], b); return new w(this.field, a)
    }
    ...
    constructor(a, b, c) {
      this.primitive = a; this.size = b; this.generatorBase = c; this.expTable = Array(this.size); this.logTable = Array(this.size); a = 1; for (b = 0; b < this.size; b++) this.expTable[b] = a, a *= 2, a >= this.size && (a = (a ^ this.primitive) & this.size - 1); for (a = 0; a < this.size - 1; a++) this.logTable[this.expTable[a]] = a; this.zero = new w(this, Uint8ClampedArray.from([0])); this.one = new w(this, Uint8ClampedArray.from([1]))
    }
...
- this.logTable[a] - 1]
    ...
    buildMonomial(a, b) { if (a < 0) throw Error('Invalid monomial degree less than 0'); if (b === 0) return this.zero; a = new Uint8ClampedArray(a + 1); a[0] = b; return new w(this, a) }
