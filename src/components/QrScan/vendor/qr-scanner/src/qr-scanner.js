export default class QrScanner {
  ...
  _scanFrame() {
    ...
    requestAnimationFrame(() => {
      ...
      this._qrEnginePromise
        .then((qrEngine) => QrScanner.scanImage(
          this.$video,
          this._scanRegion,
          qrEngine,
          this.$canvas
  ...
  _getCameraStream() {
    ...
    const preferenceType = this._preferredCamera === 'environment'
    ...
    const constraintsWithCamera = constraintsWithoutCamera.map((constraint) => ({ ...constraint, [preferenceType]: { exact: this._preferredCamera }, }));
    ...
    return [...constraintsWithCamera, ...constraintsWithoutCamera].reduceRight(
      (fallback, constraint) => () => navigator.mediaDevices
