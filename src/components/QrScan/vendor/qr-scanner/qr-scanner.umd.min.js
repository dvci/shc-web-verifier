(function (d, a) {
  ...
  class d {
    static hasCamera() {
      return navigator.mediaDevices
        ? navigator.mediaDevices
            .enumerateDevices()
            .then((a) => a.some((a) => a.kind === "videoinput"))
            .catch(() => !1)
        : Promise.resolve(!1);
    }
    ...
    constructor(
      ...
      typeof e === "number"
        ? ((this._legacyCanvasSize = e),
          console.warn(
            "You're using a deprecated version of the QrScanner constructor which will be removed in the future"
          ))
        : (this._calculateScanRegion = e);
    ...
    start() {
      ...
      return this._getCameraStream(a, !0)
        .catch(() => {
          a = a === "environment" ? "user" : "environment";
        ...
        this._offTimeout ||
          (this._offTimeout = setTimeout(() => {
            const a = this.$video.srcObject
              ...
              : [];
            for (const b of a) b.stop();
            this._offTimeout = this.$video.srcObject = null;
          }, 300)));
    }

    static scanImage(a, b = null, c = null, e = null, f = !1, k = !1) {
      const h = c instanceof Worker;
      let g = Promise.all([c || d.createQrEngine(), d._loadImage(a)]).then(
        ([a, g]) => {
          c = a;
          let k;
          ...
          return c instanceof Worker
            ? (h || c.postMessage({ type: "inversionMode", data: "both" }),
              new Promise((a, b) => {
                ...
                const h = k.getImageData(0, 0, e.width, e.height);
                c.postMessage({ type: "decode", data: h }, [h.data.buffer]);
              }))
            : new Promise((a, b) => {
                const f = setTimeout(() => b("Scanner error: timeout"), 1e4);
                c.detect(e)
                  .then((c) => {
      ...
      requestAnimationFrame(() => {
        this.$video.readyState <= 1
          ...
          : this._qrEnginePromise
              .then((a) =>
                d.scanImage(this.$video, this._scanRegion, a, this.$canvas)
              )
              .then(this._onDecode, (a) => {
                this._active &&
                  ((a.message || a).indexOf("service unavailable") !== -1 &&
                    (this._qrEnginePromise = d.createQrEngine()),
    ...
    static _drawToCanvas(a, b = null, c = null, e = !1) {
      ...
      const d = b && b.x ? b.x : 0;
      const k = b && b.y ? b.y : 0;
      const h = b && b.width ? b.width : a.width || a.videoWidth;
      const g = b && b.height ? b.height : a.height || a.videoHeight;
      e ||
    ...
    static _awaitImageLoad(a) {
      return new Promise((b, c) => {
        if (a.complete && a.naturalWidth !== 0) b();
        else {
          let d;
          let f;
          d = () => {
